...
GOAL_POSE_Q = transforms3d.quaternions.axangle2quat(np.array([0, 1, 0]), theta=np.deg2rad(90))
...
class SequentialTaskEnv(SceneManipulationEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert all_equal([len(plan.subtasks) for plan in task_plans]), "All parallel task plans must be the same length"
        assert all(
            [all_same_type(parallel_subtasks) for parallel_subtasks in zip(*[plan.subtasks for plan in task_plans])]
        ...
        self._require_build_configs_repeated_equally_across_envs = require_build_configs_repeated_equally_across_envs
        ...
        self.base_task_plans = dict((tuple([subtask.uid for subtask in tp.subtasks]), tp) for tp in task_plans)
    ...
    def _merge_pick_subtasks(self, subtask_num: int, parallel_subtasks: List[PickSubtask]):
        merged_obj_name = f"obj_{subtask_num}"
        self.subtask_objs.append(self._create_merged_actor_from_subtasks(parallel_subtasks, name=merged_obj_name))
        ...
        if all([subtask.articulation_config is not None for subtask in parallel_subtasks]):
            merged_articulation_name = f"articulation-{subtask_num}"
            merged_articulation = self._create_merged_articulation_from_articulation_ids(
                [subtask.articulation_config.articulation_id for subtask in parallel_subtasks],
    ...
    def _merge_place_subtasks(
        ...
        merged_goal_pos = common.to_tensor([subtask.goal_pos for subtask in parallel_subtasks])
        merged_goal_rectangle_corners = common.to_tensor(
            [subtask.goal_rectangle_corners for subtask in parallel_subtasks]
        )

        self.subtask_goals[-1].set_pose(Pose.create_from_pq(q=GOAL_POSE_Q, p=merged_goal_pos[env_idx]))
    ...
    def _merge_open_subtasks(self, subtask_num: int, parallel_subtasks: List[OpenSubtask]):
        ...
        assert all_equal([subtask.articulation_handle_link_idx for subtask in parallel_subtasks])
        assert all_equal([subtask.articulation_handle_active_joint_idx for subtask in parallel_subtasks])
        ...
        merged_articulation_relative_handle_pose = Pose.create_from_pq(
            p=[subtask.articulation_relative_handle_pos for subtask in parallel_subtasks]
        )
    ...
    def _merge_close_subtasks(self, subtask_num: int, parallel_subtasks: List[CloseSubtask]):
        subtask0 = parallel_subtasks[0]
        ...
        if subtask0.remove_obj_id is not None:
            ...
            ).set_pose(Pose.create_from_pq(p=[-10_000, -10_000, -9000]))  # assume one entity per scene
    ...
    def process_task_plan(
        ...
        for subtask_num, parallel_subtasks in enumerate(zip(*sampled_subtask_lists)):
            ...
            if isinstance(subtask0, PickSubtask):
                ...
                self._merge_navigate_subtasks(env_idx, last_subtask0, subtask_num, parallel_subtasks)
    ...
    def _create_merged_actor_from_obj_ids(
        ...
        merged_obj = Actor.create_from_entities(
            [self._get_actor_entity(actor_id=f"env-{i}_{oid}", env_num=i) for i, oid in enumerate(obj_ids)],
            scene=self.scene,
    ...
    def _create_merged_actor_from_subtasks(
        ...
        return self._create_merged_actor_from_obj_ids([subtask.obj_id for subtask in parallel_subtasks], name)
    ...
    def _get_articulation_entity(self, articulation_id: str, env_num: int):
        ...
        return ms_articulation._objs[ms_articulation._scene_idxs.tolist().index(env_num)]
    ...
    def _create_merged_articulation_from_articulation_ids(
        ...
        for env_num, aid in enumerate(articulation_ids):
            scene_idx_to_physx_articulation_objs[env_num] = self._get_articulation_entity(
                f"env-{env_num}_{aid}", env_num
            )
    ...
    def _load_scene(self, options):
        ...
        for subtask_num, subtask in enumerate(self.tp0.subtasks):
            if isinstance(subtask, PlaceSubtask):
                goal = self._make_goal(
                    ...
                    goal_type=("cylinder" if self.place_cfg.goal_type == "zone" else self.place_cfg.goal_type),
                )
            elif isinstance(subtask, OpenSubtask) or isinstance(subtask, CloseSubtask):
                goal = self._make_goal(
                    radius=0.05,
        ...
        for bc in self.bc_to_task_plans.keys():
            self.build_config_idx_to_task_plans[self.scene_builder.build_config_names_to_idxs[bc]] = (
                self.bc_to_task_plans[bc]
            )

        num_bcis = len(self.build_config_idx_to_task_plans.keys())

        assert not self._require_build_configs_repeated_equally_across_envs or self.num_envs % num_bcis == 0, (
        ...
        self.num_task_plans_per_bci = torch.tensor(
            [len(self.build_config_idx_to_task_plans[bci]) for bci in self.build_config_idxs],
            device=self.device,
        )
        self.scene_builder.build(self.build_config_idxs, self._init_config_names)
        self.ee_rest_pos_wrt_base = Pose.create_from_pq(p=self.EE_REST_POS_WRT_BASE, device=self.device)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options):
        with torch.device(self.device):
            ...
            self.init_config_idxs = [
                self.scene_builder.init_config_names_to_idxs[tp.init_config_name] for tp in sampled_task_plans
            ]
    ...
    def set_state_dict(self, state_dict: Dict):
        ...
        assert torch.all(torch.tensor(self.build_config_idxs) == torch.tensor(build_config_idxs)), (
        ...
        self._initialize_episode(torch.arange(self.num_envs), options=dict(task_plan_idxs=task_plan_idxs))

        assert torch.all(torch.tensor(self.init_config_idxs) == torch.tensor(init_config_idxs)), (
    ...
    def evaluate(self):
        robot_force = self.agent.robot.get_net_contact_forces(self.force_articulation_link_ids).norm(dim=-1).sum(dim=-1)
        self.robot_cumulative_force += robot_force

        # NOTE (arth): update ee_rest_world_pose every step since robot moves
        self.ee_rest_world_pose: Pose = self.agent.base_link.pose * self.ee_rest_pos_wrt_base
        ...
        progressive_task_success, progressive_task_checkers = self._progressive_task_check_success()

        success_checkers["cumulative_force_within_limit"] |= self.subtask_pointer >= len(self.task_plan)

        move_to_next_subtask = (
            subtask_success & success_checkers["cumulative_force_within_limit"] & progressive_task_success
        )
        self.subtask_pointer[move_to_next_subtask] += 1
        success = (self.subtask_pointer >= len(self.task_plan)) & progressive_task_success
        # set robot_cumulative_force to 0 if evaluating new subtask
        self.robot_cumulative_force[move_to_next_subtask & (self.subtask_pointer < len(self.task_plan))] = 0
        ...
        subtask_type = torch.full_like(self.subtask_pointer, UNIQUE_SUCCESS_SUBTASK_TYPE)
    ...
    def _progressive_task_check_success(self):
        ...
        progressive_task_success = torch.ones(self.num_envs, device=self.device, dtype=torch.bool)
        ...
        for subtask_num in self.check_progressive_success_subtask_nums:
            ...
            if isinstance(subtask, PlaceSubtask):
                subtask_progressive_success, subtask_progressive_checkers = self._place_check_success(
            ...
            for k, v in subtask_progressive_checkers.items():
                ...
                if new_k not in progressive_task_checkers:
                    progressive_task_checkers[new_k] = torch.zeros(self.num_envs, device=self.device, dtype=v.dtype)
    ...
    def _subtask_check_success(self):
        subtask_success = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
        success_checkers = dict()

        currently_running_subtasks = torch.unique(torch.clip(self.subtask_pointer, max=len(self.task_plan) - 1))
        for subtask_num in currently_running_subtasks:
            ...
            if isinstance(subtask, PickSubtask):
                ...
                raise NotImplementedError(f"{subtask.type} {type(subtask)} not supported")

            for k, v in subtask_success_checkers.items():
                if k not in success_checkers:
                    success_checkers[k] = torch.zeros(self.num_envs, device=self.device, dtype=v.dtype)
    ...
    def _pick_check_success(
        ...
        robot_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 3:-2] - self.resting_qpos)
        robot_rest = torch.all(robot_rest_dist < self.pick_cfg.robot_resting_qpos_tolerance_grasping, dim=1)
        is_static = self.agent.is_static(threshold=0.2, base_threshold=0.05)[env_idx]
        cumulative_force_within_limit = (
            self.robot_cumulative_force[env_idx] < self.pick_cfg.robot_cumulative_force_limit
        )
        ...
        if self._add_event_tracker_info:
            subtask_checkers["robot_target_pairwise_force"] = torch.norm(
                self.scene.get_pairwise_contact_forces(self.agent.finger1_link, obj)[env_idx],
                dim=1,
            ) + torch.norm(
                self.scene.get_pairwise_contact_forces(self.agent.finger2_link, obj)[env_idx],
                dim=1,
            )
        return (
            is_grasped & ee_rest & robot_rest & is_static & cumulative_force_within_limit,
            subtask_checkers,
        )
    ...
    def _place_check_success(
        ...
        if self.place_cfg.goal_type == "zone":
            ...
            xy_correct = (0 <= AM_dot_AB) & (AM_dot_AB <= AB_dot_AB) & (0 <= AM_dot_AD) & (AM_dot_AD <= AD_dot_AD)
            z_correct = (
                torch.abs(obj.pose.p[env_idx, 2] - obj_goal.pose.p[env_idx, 2]) <= self.place_cfg.obj_goal_thresh
            )
            ...
            raise NotImplementedError(f"{self.place_cfg.goal_type=} is not yet supported")
        ...
        robot_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 4:-2] - self.resting_qpos[1:])
        robot_rest = torch.all(robot_rest_dist < self.place_cfg.robot_resting_qpos_tolerance, dim=1) & (
            torch.abs(self.agent.robot.qpos[env_idx, 3] - self.resting_qpos[0]) < 0.01
        )
        is_static = self.agent.is_static(threshold=0.2, base_threshold=0.05)[env_idx]
        cumulative_force_within_limit = (
            self.robot_cumulative_force[env_idx] < self.place_cfg.robot_cumulative_force_limit
        )
        subtask_checkers = dict(
            is_grasped=is_grasped,
        ...
        return (
            ~is_grasped & obj_at_goal & ee_rest & robot_rest & is_static & cumulative_force_within_limit,
            subtask_checkers,
        )
    ...
    def _is_navigated_close(
        ...
        if isinstance(goal, Articulation) or isinstance(goal, Link):
            ...
            relative_pos_world = self.agent.base_link.pose.p[env_idx] - goal.pose.p[env_idx]
            ...
            xrange = dict(fridge=[0.933, 1.833], kitchen_counter=[0.3, 1.5])[articulation_type]
            yrange = dict(fridge=[-0.6, 0.6], kitchen_counter=[-0.6, 0.6])[articulation_type]
    ...
    def _navigate_check_success(
        ...
        oriented_correctly = torch.abs(rots) <= self.navigate_cfg.navigated_successfully_rot

        navigated_close = self._is_navigated_close(env_idx, goal, articulation_type)
        is_static = self.agent.is_static(threshold=0.2, base_threshold=0.05)[env_idx]

        cumulative_force_within_limit = (
            self.robot_cumulative_force[env_idx] < self.navigate_cfg.robot_cumulative_force_limit
        )

        if self.navigate_cfg.ignore_arm_checkers:
            return (
                oriented_correctly & navigated_close & is_static & cumulative_force_within_limit,
                dict(
                    is_grasped=is_grasped,
        ...
        if obj is None:
            robot_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 4:-2] - self.resting_qpos[1:])
            torso_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 3] - self.resting_qpos[0])
            ...
            robot_rest = torch.all(robot_rest_dist < self.navigate_cfg.robot_resting_qpos_tolerance, dim=1) & (
                torso_rest_dist < 0.01
            )
            # TEMPORARILY RELAX JOINT 8 (joint 8 = qpos[12]
            # joint_tolerances = torch.full_like(robot_rest_dist, self.navigate_cfg.robot_resting_qpos_tolerance)
            # joint_tolerances[:, 8] = 0.5  # Relax wrist_roll tolerance
            # robot_rest = torch.all(robot_rest_dist < joint_tolerances, dim=1) & (torso_rest_dist < 0.01)
        else:
            robot_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 3:-2] - self.resting_qpos)
            ...
            robot_rest = torch.all(
                robot_rest_dist < self.navigate_cfg.robot_resting_qpos_tolerance_grasping,
                dim=1,
            )
            if len(obj._scene_idxs) != self.num_envs:
                subtask_envs_with_obj = tensor_intersection_idx(env_idx, obj._scene_idxs)
                robot_rest[subtask_envs_with_obj] &= torch.all(
                    robot_rest_dist[subtask_envs_with_obj] < self.navigate_cfg.robot_resting_qpos_tolerance,
                    dim=1,
                )

        navigate_success = oriented_correctly & navigated_close & ee_rest & robot_rest & is_static
        if obj is not None:
            if len(obj._scene_idxs) != self.num_envs:
                subtask_envs_with_obj = tensor_intersection_idx(env_idx, obj._scene_idxs)
                navigate_success[subtask_envs_with_obj] &= is_grasped[subtask_envs_with_obj]
            else:
    ...
    def _open_check_success(
        ...
        is_grasped = self.agent.is_grasping(articulation.links[link_idx], max_angle=30)[env_idx]
        articulation_open = articulation.qpos[env_idx, active_joint_idx] > (
            (articulation.qlimits[env_idx, active_joint_idx, 1] - articulation.qlimits[env_idx, active_joint_idx, 0])
        ...
        robot_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 4:-2] - self.resting_qpos[1:])
        robot_rest = torch.all(robot_rest_dist < self.open_cfg.robot_resting_qpos_tolerance, dim=1) & (
            torch.abs(self.agent.robot.qpos[env_idx, 3] - self.resting_qpos[0]) < 0.01
        )
        is_static = self.agent.is_static(threshold=0.2, base_threshold=0.05)[env_idx]
        cumulative_force_within_limit = (
            self.robot_cumulative_force[env_idx] < self.open_cfg.robot_cumulative_force_limit
        )
        subtask_checkers = dict(
            is_grasped=is_grasped,
        ...
        if self._add_event_tracker_info:
            subtask_checkers["handle_active_joint_qpos"] = articulation.qpos[env_idx, active_joint_idx]
            subtask_checkers["handle_active_joint_qmin"] = articulation.qlimits[env_idx, active_joint_idx, 0]
            subtask_checkers["handle_active_joint_qmax"] = articulation.qlimits[env_idx, active_joint_idx, 1]
            subtask_checkers["robot_target_pairwise_force"] = torch.norm(
                self.scene.get_pairwise_contact_forces(self.agent.finger1_link, articulation.links[link_idx])[env_idx],
                dim=1,
            ) + torch.norm(
                self.scene.get_pairwise_contact_forces(self.agent.finger2_link, articulation.links[link_idx])[env_idx],
                dim=1,
            )
        return (
            articulation_open & ee_rest & robot_rest & is_static & cumulative_force_within_limit,
            subtask_checkers,
        )
    ...
    def _close_check_success(
        ...
        robot_rest_dist = torch.abs(self.agent.robot.qpos[env_idx, 4:-2] - self.resting_qpos[1:])
        robot_rest = torch.all(robot_rest_dist < self.close_cfg.robot_resting_qpos_tolerance, dim=1) & (
            torch.abs(self.agent.robot.qpos[env_idx, 3] - self.resting_qpos[0]) < 0.01
        )
        is_static = self.agent.is_static(threshold=0.2, base_threshold=0.05)[env_idx]
        cumulative_force_within_limit = (
            self.robot_cumulative_force[env_idx] < self.close_cfg.robot_cumulative_force_limit
        )
        subtask_checkers = dict(
            is_grasped=is_grasped,
        ...
        return (
            articulation_closed & ee_rest & robot_rest & is_static & cumulative_force_within_limit,
            subtask_checkers,
        )
        ...
        for subtask_num in currently_running_subtasks:
            ...
            if self.subtask_objs[subtask_num] is not None:
                if len(self.subtask_objs[subtask_num]._scene_idxs) != self.num_envs:
                    env_scene_idx = tensor_intersection(env_idx, self.subtask_objs[subtask_num]._scene_idxs)
                    obj_pose_wrt_base[env_scene_idx] = vectorize_pose(
                        base_pose_inv[env_scene_idx] * self.subtask_objs[subtask_num].pose
                    )
                else:
            ...
            if self.subtask_goals[subtask_num] is not None:
                if isinstance(subtask, OpenSubtask) or isinstance(subtask, CloseSubtask):
                    goal_pos_wrt_base[env_idx] = (base_pose_inv * self.handle_world_poses[subtask_num]).p[env_idx]
                else:
                    goal_pos_wrt_base[env_idx] = (base_pose_inv * self.subtask_goals[subtask_num].pose).p[env_idx]
